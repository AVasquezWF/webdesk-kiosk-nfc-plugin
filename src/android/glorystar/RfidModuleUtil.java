...
public class RfidModuleUtil {
                    ...
                    if (str.length() >10) {
                        ...
                        if (str.trim().substring(0,4).equals("0001")){
                            switch (str.substring(4,6)){
                                case "80":
                                    cardType = HFTAG_MIFARE;
                                    cardId = str.substring(10);
                                    break;
                                case "84":
                                    cardType = HFTAG_HIDICLASS;
                                    cardId = str.substring(10);
                                    break;
                                case "40":
                                    cardType = LFTAG_EM4102;
                                    cardId = str.substring(10);
                                    break;
                                case "49":
                                    cardType = LFTAG_HIDPROX;
                                    cardId = str.substring(10);
                                    break;
                                default:
                                    if (cardType.equals(HFTAG_MIFARE)){
                                        cardValue = str.substring(9);
                                    }else if (cardType.equals(HFTAG_HIDICLASS)){
    ...
    public void searchTag(){
        cardType = "";
        thread.sendCmds(SEARCH_TAG.getBytes());
    }

    public void readTag(){
        switch (cardType){
            case HFTAG_MIFARE:
                thread.sendCmds(MIFARE_LOGIN.getBytes());
                thread.sendCmds(MIFARE_READ.getBytes());
                break;
            case HFTAG_HIDICLASS:
                thread.sendCmds(ICLASS_READ.getBytes());
                break;
            case LFTAG_EM4102:
                break;
            case LFTAG_HIDPROX:
